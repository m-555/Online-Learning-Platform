...
import type { Dashboard, UserAccount } from "@/lib/types";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.dashboard(token), api.me(token)])
      .then(([dashboard, me]) => {
        setData(dashboard);
        setUser(me);
        setStatus("ready");
  ...
  return (
    <div className="flex min-h-screen bg-canvas">
      <DashboardSidebar messagesCount={stats.messages_count} />
      <div className="flex min-w-0 flex-1 flex-col">
        <DashboardTopbar studentName={data.student_name} avatarColor="#0E9F6E" />
